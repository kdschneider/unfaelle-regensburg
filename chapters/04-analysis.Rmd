...
```{r unfaelle-monthly, fig.height = 6}
plot.bar.month_1 / 
  plot.bar.month_2 / 
  plot.bar.month_3 + 
  plot_annotation(
...
Die meisten Unfälle mit personenschade finden tagsüber statt. Zusätzlich zeigt Abbildung \@ref(fig:unfaelle-time) zwei Peaks: Um sieben Uhr und von 16-17 Uhr. Diese stimmen mit dem Arbeitsverkehr überein.

Abbildung \@ref(fig:unfaell-time) zeigt auch, dass Unfälle bei Dunkelheit zwischen 16 und 6 Uhr stattfinden. Das ist keine überraschende Erkenntnis, stärkt aber das Vertrauen in die Zuverlässigkeit der Daten.

```{r unfaelle-time, fig.height = 6}
plot.bar.time_1 / 
  plot.bar.time_2 / 
  plot.bar.time_3 +
  plot_annotation(
...
Abbildung \@ref(fig:unfaell-district1) zeigt die Anzahl an Unfällen von `r min(data$year)` und `r max(data$year)`. In diesem Zeitraum haben sich die meisten Unfälle in der Regensburger Innenstadt (`r filter(data.district_plot, district == "Innenstadt")$n`) zugetragen.

Berechnet man die Unfälle pro Quadratkilometer, wird der Unterschied zwischen der Innenstadt und den Umliegenden Stadtteilen noch eindeutiger (siehe Abbildung \@ref(unfaelle-distirct2)).

```{r unfaelle-district1}
plot.district_total_bar + 
  inset_element(
    plot.district_total_map, 
    left = 0.35, 
